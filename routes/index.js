...
router.post("/unsubscribe", async (req, res) => {
  ...
  subscribers = subscribers.filter((subs) => {
...
router.post("/notify/all", async (req, res) => {
  ...
  const clickTarget =
    req.fields.clickTarget || "http://oscarcode.herokuapp.com/";
  ...
  client.putFile(
    ...
    function (err, response) {
      ...
      fs.writeFile(filepath, fileContent, (err) => {
        ...
        conn.connect();
        conn.query(
          `select max(idPost) maxNum from Post;`,
          function (err, rows, fields) {
            if (err) {
              ...
              console.log(numMax);
              conn.query(
                `insert into Post values(${numMax},'${titulo}',0,sysdate(), '${url}','${descripcion}', '${postName}');`,
                function (err, rows, fields) {
                  if (err) {
                    conn.end();
